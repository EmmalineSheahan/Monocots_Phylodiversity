...
save_SDM_results <- function(ENMeval_output, AUCmin, resultDir, spp, occ_df){
  ...
  print(paste("Here is resultDir ", resultDir, sep = ":"))
  ### Find the best model
  bestmod <- ENMeval_output@results %>% dplyr::filter(delta.AICc == 0) 
  ## if there are two best models then choose one from it, by checking the max value
  ## from auc.train
  
  if (nrow(bestmod) > 1)
  {
    modelNumber <- which.max(bestmod$auc.train)
    bestmod <- bestmod[modelNumber,]
  }
  ...
  print(paste("Saving the best model in a file  ", paste0(resultDir, "/", spp, "_bestModel.csv"), sep = ":"))
  ...
  ## raster::writeRaster(x = r_best, filename = paste0(resultDir, "/", spp, "_SDM.asc"), overwrite = TRUE)
  ...
  ## When all the three tss_vals are same, then choose the first one. 
  if (length(lpt_10) > 1)
  {
  ...
  pa_mat <- matrix(pa, ncol = 3, byrow = TRUE)
  r_best_pa <- terra::classify(r_best, pa_mat)
  print(paste("This is the PA file name ", paste0(resultDir, "/", spp, "_SDM_PA.tif"), sep = ":"))
  ...
  print(paste("This is the Variable Importance file name ", paste0(resultDir, "/", spp, "_variableImportance.csv"), sep = ":"))
